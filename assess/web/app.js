...
function header() {
  return `<header><div class="brand">nua<span>fixed science demonstration</span></div><nav aria-label="Main navigation"><a href="index.html">Mangal Inquiry</a><button data-view="home">Overview</button><button data-view="report">Teacher report</button><button data-view="settings">Settings</button></nav></header>`;
}
function footer() {
  return `<footer><span>Nua Assess · MVP 0.3.0 · Science demo</span><span>Stored on this device · No cloud AI · Educational validation pending</span></footer>`;
}
function home() {
