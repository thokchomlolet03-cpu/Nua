# MVP checkpoint — 0.6.0 — 2026-09-14

Read [SYSTEMATIC-UPDATE-0.6.0.md](SYSTEMATIC-UPDATE-0.6.0.md) for the test-to-change trace, [COMPETITION-READINESS.md](COMPETITION-READINESS.md) for current competition scope and [PILOT-PROTOCOL.md](PILOT-PROTOCOL.md) for remaining evidence gates. Added evidence-linked educator feedback, optional learner uncertainty signals, locally saved educator drafts, separate follow-up responses and minimized feedback exports. All 20+ inquiry questions remain required. The legacy overall annotation is retained inside an optional disclosure; new feedback uses the evidence-linked workflow.

This update converts the controlled comparison findings into a bounded learner-flow release. Generated questions now carry an explicit, editable response contract; the learner sees that contract one question at a time and may use short sentences, labelled lists or tables. Relevant plain-language glossary terms are disclosed without supplying conclusions. The 20-question minimum and all 24 registered reasoning types remain intact. The question scaffolds are generic to the uploaded material; synthetic plant-specific wording is not embedded in the production templates.

The inquiry journey is presented in three navigable stopping points: orient/reconstruct (questions 1–8), inspect evidence/diagnose (9–16), and transfer/generate inquiry (17 onward). This is pacing support, not a measured focus intervention. Source anchors are selected as complete usable sentences or bounded exact source spans rather than blindly taking the first 240 characters. Local-AI drafted questions receive the same deterministic response contract and glossary metadata; model output still cannot approve an item.

89 Node tests pass. Android assembly, lint and JVM tests pass for version 0.6.0 / code 7; lint reports six non-blocking warnings and no errors. The debug APK is 53 MB with SHA-256 `7324d0aa6511020f827bb3eb7b5ae2b8088393c8d573be7d0642eebd2f63a0de`. No physical phone is connected; runtime performance, native model use and educational validity remain unverified.
...
Final release check from 0.5.0: reopening after the waiting offline update installed `0.5.0-r2` retained the synthetic evidence and showed the collapsed legacy annotation. The 0.6.0 offline cache is now pinned separately so an older open tab cannot mix assets. The new UI still requires a fresh browser load before the new cache activates.
