...
class WavUtilsTest {
    ...
    @Test
    fun testWavFuzzing() {
        val tempFile = File.createTempFile("wav_fuzz_", ".wav")
        try {
            val random = java.util.Random(42)
            for (size in listOf(0, 1, 4, 8, 12, 16, 44, 100, 1000)) {
                for (i in 0 until 50) {
                    ...
                    try {
                        WavUtils.findDataChunkOffset(tempFile)
                    } catch (e: Exception) {
                        assertTrue("Unexpected exception: ${e::class.java.name}", e is IllegalArgumentException || e is java.io.IOException)
                    }
                }
            }
        } finally {
